...
from mesh_utils import get_geom_limits
import itertools
...
v_fields = {
    'v_u': torch.from_numpy(V_DISP),
    'v_e': torch.from_numpy(V_STRAIN)
}
...
with torch.no_grad():
    ...
    for i, df in enumerate(df_list):
        ...
        f = torch.from_numpy(df[['fxx_t','fyy_t']].values.reshape(-1,2))

        a = torch.from_numpy(df['area'].values.reshape(-1,1))
        ...
        f = f.reshape(n_tps,n_elems,-1).permute(1,0,2)[0]
        a = a.reshape(n_tps,n_elems,-1).permute(1,0,2)[:,0].unsqueeze(-1)
        
        w_int_ann = internal_vw(s, v_fields['v_e'].unsqueeze(2), a)
        w_int = internal_vw(y, v_fields['v_e'].unsqueeze(2), a)
        w_ext = external_vw(f, v_fields['v_u'].unsqueeze(1))

        vars = ['w_int','w_int_ann','w_ext']
        cols = [[f'{var}_{vf}' for vf in range(w_ext.shape[0])] for var in vars]
        ...
        res = np.concatenate([*w_int,*w_int_ann,*w_ext], axis=1)
            
        results = pd.DataFrame(res, columns=cols)
            
        results.to_csv(os.path.join(DATA_DIR, f'{tag}_global_w.csv'), header=True, sep=',', float_format='%.10f')
