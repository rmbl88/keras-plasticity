...
def plot():    #Function to create the base plot, make sure to make global the lines, axes, canvas and any part that you would want to update later
    ...
    global w_ext,w_int,w_int_real,t_loss,v_loss,ax,ax_2,canvas,backgrounds
    ...
    try:       #Try to check if there is data in the queue
        ...
        if result != 'Q':
            if tag == 'loss':
                ...
                lower = min([min(a) for a in [d1,d2]])
                upper = max([max(a) for a in [d1,d2]])
...
def train(q):
    ...
    def get_sbvfs(model, dataloader, isTrain = True):
        ...
        with torch.no_grad():
            ...
            for batch in range(num_batches):
                ...
                x, _, _, _, tag, inc = dataloader[batch]

                tags = tag[::n_elems].values.tolist()
                incs = inc[::n_elems].values.tolist()

                if dataloader.shuffle == True:
                    idx = torch.sort(torch.tensor(incs)).indices
                else:
                    idx = None
                ...
                if isTrain:
                    ...
                    s = mdl_(torch.from_numpy(dataloader.scaler_x.transform(x)))

                ds = sigma_deltas(model, x.detach(), s.detach(), param_dicts, idx, incs)
                
                # Computing sensitivity-based virtual fields
                v_u, v_disp, v_strain = sbv_fields(ds, b_glob, b_inv, n_elems, bcs, active_dof)               

                for i,(n,j) in enumerate(tuple(zip(tags,incs))):
                    if isTrain:
                        VFs_train[n]['u'][j] = v_u[:,i].detach().numpy()
                        VFs_train[n]['v_u'][j] = v_disp[:,i].detach().numpy()
                        VFs_train[n]['e'][j] = torch.reshape(v_strain,[n_vfs,t_pts,n_elems,3])[:,i].detach().numpy()
                    else:
                        VFs_test[n]['u'][j] = v_u[:,i].detach().numpy()
                        VFs_test[n]['v_u'][j] = v_disp[:,i].detach().numpy()
                        VFs_test[n]['e'][j] = torch.reshape(v_strain,[n_vfs,t_pts,n_elems,3])[:,i].detach().numpy()
    ...
    def train_loop(dataloader, model, loss_fn, optimizer):
        ...
        for batch in range(num_batches):
            ...
            tag = tags[0]
            v_disp = torch.from_numpy(np.array(list(VFs_train[tag]['v_u'].values()))).permute((1,0,2,3))
            v_disp = v_disp[:,incs]
            n_vfs = v_disp.shape[0]
            v_strain = torch.from_numpy(np.array(list(VFs_train[tag]['e'].values()))).permute((1,0,2,3))
            ...
            # l_jac_weight = 0.5
            # l_jac = l_jac_weight * torch.nn.functional.relu(-batch_jacobian(model,X_train)).sum()
            ...
            # lw_ = torch.mean(torch.square(s[0]))
        ...
        #-----------------------------
        return losses, v_work_real, np.mean(g_norm)
    ...
    def test_loop(dataloader, model, loss_fn):
        ...
        with torch.no_grad():
            ...
            for batch in range(num_batches):
                ...
                # Loading sensitivity-based virtual fields
                tag = tags[0]
                v_disp = torch.from_numpy(np.array(list(VFs_test[tag]['v_u'].values()))).permute((1,0,2,3))
                n_vfs = v_disp.shape[0]
                v_strain = torch.from_numpy(np.array(list(VFs_test[tag]['e'].values()))).permute((1,0,2,3))
                v_strain = torch.reshape(v_strain,(n_vfs,pred.shape[0],pred.shape[1]))
                #-----------------------------------------------------------------------------------------
...
#----------------------------------------------------    
    ...
    train_generator = DataGenerator(X, y, f, coord, info, partition["train"], batch_size, shuffle=True, std=True, t_pts=T_PTS)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=50, cooldown=20, factor=0.2, min_lr=1e-5)
    ...
    for t in range(epochs):
        ...
        if t==0:
            ...
            get_sbvfs(copy.deepcopy(model_1), train_generator)
        ...
        #--------------------------------------------------------------
        batch_losses, batch_v_work, grad_norm = train_loop(train_generator, model_1, loss_fn, optimizer)
        #--------------------------------------------------------------        
        ...
        if t!= 0:
            tol_updt = (train_loss[0]/1.5**k_opt)
    ...
    np.save(output_logs + 'sbvfs.npy', VFs_train_best)
    np.save(output_logs + 'w_virt.npy', w_virt_best)
