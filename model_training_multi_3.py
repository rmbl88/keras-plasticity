...
def jacobian(y, x, create_graph=False):                                                               
    jac = []                                                                                          
    flat_y = y.reshape(-1)                                                                            
    grad_y = torch.eye(3).reshape(1,3,3).repeat(9,1,1)                                                                 
...
def get_v_fields(cent_x, cent_y, x, y):
    ...
    virtual_disp = {
        ...
        44: torch.stack([(2*math.pi*y/LENGTH**2)*torch.sin(2*math.pi*x/LENGTH),(-y/LENGTH)*torch.sin(math.pi*x/LENGTH)**2],1),
    ...
    virtual_strain = {
        ...
        31: torch.stack([zeros,torch.sin(cent_x*math.pi/(LENGTH))**2/LENGTH,(math.pi*cent_y)*torch.sin(2*cent_x*math.pi/LENGTH)/LENGTH**2],1),
        ...
        44: torch.stack([(4*math.pi**2*cent_y/LENGTH**3)*torch.cos(2*math.pi*cent_x/LENGTH),(-1/LENGTH)*torch.sin(math.pi*cent_x/LENGTH)**2, zeros],1),
...
def train_loop(dataloader, model, loss_fn, optimizer):
    ...
    for batch in range(num_batches):
        ...
        x = LENGTH * torch.ones(n_surf_nodes, dtype=torch.float64)
        y = torch.tensor(np.linspace(0, LENGTH, n_surf_nodes), dtype=torch.float64)
        ...
        # dsde = jacobian(pred,X_train,create_graph=True)
