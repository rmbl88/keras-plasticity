...
# ---------------------------------------------------------------    
    ...
    if config.wandb.log:
        ...
        tags = [f'alpha_{config.train.loss_settings.alpha}', f'beta_{config.train.loss_settings.beta}', f'tau_{config.train.loss_settings.tau}', f'batch_{BATCH_DIVIDER}', f'cholesky_{config.model.cholesky}'] + list(config.train.losses.values()) + [f'{a[0]}_bias_{a[1]}' for a in list(zip(config.model.bias.keys(),config.model.bias.values()))] + [f'{config.vfm.type}']
...
# ---------------------------------------------------------------
    ...
    if config.model.init.pre_train:
        param_dict = get_ann_model(config.model.init.pre_trained_run,config.model.init.pre_train_dir)
    ...
    if config.vfm.type == 'sbvf':  # Sensitivity-based VFM
    ...
    elif config.vfm.type == 'udvf':  # User-defined VFM
