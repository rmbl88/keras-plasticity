...
for model in ann_models:
    ...
    for i, ax in enumerate(fig.axes):
        ...
        stress_umat = [col for col in uniax_tests[directions[k]].columns if directions[k]+'_umat' in col]

        if i<3:

            ax.plot(uniax_tests[directions[k]][strain], uniax_tests[directions[k]][stress], label=r'\textbf{ABAQUS}', color='k')
            ax.plot(uniax_tests[directions[k]][strain], uniax_tests[directions[k]][stress_pred], label=r'\textbf{%s}' % model, color='royalblue')
            ax.plot(uniax_tests[directions[k]][strain], uniax_tests[directions[k]][stress_umat], label=r'\textbf{%s-UMAT}' % model, color='seagreen')
            ...
            ax.plot(err_pred, color="royalblue", marker='8', lw=0.65, markersize=2.25, markerfacecolor='white',markeredgewidth=0.35)
            ax.plot(err_umat, color="seagreen", marker='s', lw=0.65, markersize=2.25, markerfacecolor='white',markeredgewidth=0.35)
    ...
    plt.savefig(os.path.join(DIR, model, f'{DIR}_{model}.pdf'), format="pdf", dpi=600, bbox_inches='tight')
    plt.clf()
    plt.close(fig)