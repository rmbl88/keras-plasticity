...
# model_2.load_state_dict(torch.load('outputs/9-elem-200-plastic_testfull/models/[9-8x1-3]-9-elem-200-plastic-12-VFs_2.pt'))
# model_3.load_state_dict(torch.load('outputs/9-elem-200-plastic_testfull/models/[9-8x1-3]-9-elem-200-plastic-12-VFs_3.pt'))
model_1.eval()
# model_2.eval()
# model_3.eval()
...
with torch.no_grad():
    for i, df in enumerate(sampled_dfs):
        ...
        if df['id'][0]==9 or df['id'][0]==1:
            ...
            results.to_csv('outputs/9-elem-200-plastic_testfull/val/' + df['tag'][0]+'_'+str(df['id'][0])+'.csv', header=True, sep=',',float_format='%.8f')
            print('hey')
